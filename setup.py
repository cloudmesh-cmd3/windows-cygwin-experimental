...
banner("Installing cloudmesh_cmd")
...
auto_create_version("cloudmesh_cmd", version, filename="version.py")
...
class UploadToPypi(install):
    ...
    def run(self):
        auto_create_version("cloudmesh_cmd", version)
...
data_files= [ (home + '/.cloudmesh/' + d.lstrip('cloudmesh_cmd/'),
                [os.path.join(d, f) for f in files]) for d, folders, files in os.walk('cloudmesh_cmd/etc')]
...
matches = []
for root, dirnames, filenames in os.walk('cloudmesh_cmd/etc'):
  for filename in fnmatch.filter(filenames, '*'):
    matches.append(os.path.join(root, filename).lstrip('cloudmesh_cmd/'))
data_dirs = matches
...
#    app=APP,
    ...
    description="cloudmesh windows - porting cloudmesh to Windows",
    long_description=read('README.rst'),
    license="Apache License, Version 2.0",
    author="Gregor von Laszewski",
    author_email="laszewski@gmail.com",
    url="https://github.com/cloudmesh/windows",
    classifiers=[
